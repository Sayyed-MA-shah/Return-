...
class LaptopReturnTrackerUI:
    ...
    def build_ui(self):
        ...
        # Style for Comboboxes and Entries (white bg, black text)
        ...
        # ---------- Top Navigation ---------- #
        nav_frame = ttk.Frame(self.root, padding=(15, 15), style="TFrame")
        nav_frame.pack(fill="x")

        #
    
        ttk.Button(nav_frame, text="Add New Return", command=self.callbacks['show_form'],  style="Rounded.TButton").pack(side="left", padx=(0, 12))
        ttk.Button(nav_frame, text="Returns Table", command=self.callbacks['show_table'], style="Rounded.TButton").pack(side="left")

        ttk.Label(nav_frame, text="Search by ID:", font=FONT, background="#2c3e50", foreground="white").pack(side="left", padx=(20, 5))
        self.quick_search_entry = ttk.Entry(nav_frame, width=15, font=FONT)
        self.quick_search_entry.pack(side="left")

        ttk.Button(nav_frame, text="Go", command=self.callbacks['quick_search'], style="Rounded.TButton").pack(side="left", padx=(5, 0))

        self.delete = ttk.Entry(nav_frame, width=15, font=FONT)
        self.delete.pack(side="left", padx=(30, 5))
        ttk.Button(nav_frame, text="Delete", command=self.callbacks['delete'], style="Rounded.TButton").pack(side="left")
    ...
    def build_form_view(self):
        ...
        ttk.Label(form_grid, text="Inspector Notes:", font=FONT).grid(row=4, column=0, columnspan=4, sticky="w", padx=5, pady=(15, 0))
        self.notes_text = tk.Text(form_grid, height=3, font=FONT, bg="white", fg="black", insertbackground="#000000")
        self.notes_text.grid(row=5, column=0, columnspan=4, sticky="ew", padx=5, pady=(0, 10))
        ...
        actions = ttk.Frame(self.form_frame)
        actions.pack(pady=10)
        ttk.Button(actions, text="Save Return", command=self.callbacks['save'], style="Rounded.TButton").pack(side="left", padx=5)
        ttk.Button(actions, text="Reset Form", command=self.callbacks['reset'], style="Rounded.TButton").pack(side="left", padx=5)
        ttk.Button(actions, text="Back to Table View", command=self.callbacks['show_table'], style="Rounded.TButton").pack(side="left", padx=5)
    ...
    def build_table_view(self):
        ...
        ttk.Label(search_frame, text="Search by ID:", font=FONT).pack(side="left", padx=5)
        self.search_id_entry = ttk.Entry(search_frame, width=20, font=FONT)
        self.search_id_entry.pack(side="left", padx=5)

        ttk.Button(search_frame, text="Search", command=self.callbacks['search'], style="Rounded.TButton").pack(side="left", padx=5)

        ttk.Label(search_frame, text="Filter by Condition:", font=FONT).pack(side="left", padx=20)
        self.filter_condition_cb = ttk.Combobox(search_frame, values=["All", "Like New", "Good", "Fair", "Poor"], state="readonly")
        self.filter_condition_cb.current(0)
        self.filter_condition_cb.pack(side="left")
        ttk.Button(search_frame, text="Filter", command=self.callbacks['filter'], style="Rounded.TButton").pack(side="left", padx=5)
